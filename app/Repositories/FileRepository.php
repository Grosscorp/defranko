...
use App\File;
use App\Repositories\Traits\TokenGenerator;
...
//			->orderBy('created_at', 'desc');
	...
	public static function getSizes($fileType, $thumb = false) {
		...
		$defaultSizes = $config[FileRepository::FILE_DEFAULT];
...
//		}
	...
	{
		return public_path() . DIRECTORY_SEPARATOR . $this->generateFilePath($file) . Config::get('files.original_filename') . '.' . $file->ext;
	}
...
//				$image->createDimensions($toType, $resultToken, $destinationFileUrl);
		...
		$data = [
			...
			'landLordId' => ($user->getType() == UserRepository::USER_TYPE_LANDLORD) ? $user->id : null,
			'type' => $toType,
			'name' => $file['name'],
			'token' => $resultToken,
			'size' => $file['size'],
