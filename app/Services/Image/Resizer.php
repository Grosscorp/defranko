...
use App\Events\Files\ImageToBlurWasUploaded;
...
//				exec($this->convert . ' ' . $sourceUrl . ' -resize ' . $newResolution[0] . 'x' . $newResolution[1] . '! -strip -quality '. $quality .' ' . $targetUrl);
	...
	{
		exec($this->convert . ' ' . $sourceUrl  . ' -crop ' . $geometry . ' +repage -resize ' . $width . 'x' . $height . ' -strip -quality '. $quality .' ' . $targetUrl);
	}
	...
	{
		exec($this->convert . ' ' . $sourceUrl . ' -resize ' . $width . 'x' . $height . '! -strip -quality '. $quality .' ' . $targetUrl);
	}
