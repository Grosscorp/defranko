(function() {
	...
	angular.module("directivesForm", [])
		...
		.directive('quoteFileUploader', quoteFileUploader)
	...
	function customSelect() {
		...
		function templateFunc(tElement, tAttrs) {
			...
			} else if(tAttrs.asarray) {
				return '/app/views/form/select-arr.html';
			}
			else {
		...
		function controllerFunc($scope) {
			...
			} else if($scope.asarray) {
				var selected = _.findWhere($scope.options, {selected:true});
				if(selected) {
					$scope.selectedValue = selected.id;
				}
	...
	function quoteFileUploader() {
		...
		var directive = {
			require: [],
			restrict: 'E',
			templateUrl: '/app/views/form/quoteFileUploader.html',
			scope: {
				fileType: '@',
				parent: '=',
				items: '=',
			...
			controller: ['$scope', 'FileUploader', 'FilesService', 'HttpService', '$stateParams', '$http', '$state', '$location', controllerFunc],
			replace: true,
			priority: 10
		};
		...
		function controllerFunc($scope, FileUploader, FilesService, HttpService, $stateParams, $http, $state, $location) {

			//======================================================================
			// FILTERING STUFF
			$scope.filter = {};

			$scope.filter.quote_id = $stateParams.quote_id || null;
			$scope.filter.user_id = $stateParams.user_id || null;
			$scope.filter.category_id = $stateParams.category_id || null;

			$scope.$watchCollection('filter', function(obj) {
				if(obj) {
			...
			//DOCS files loader with pagination
			$scope.items = [];
			$scope.loading = false;
			...
			function loadPage() {
				...
				var params = {
					...
					take : $scope.pagination.take,
					quote_id : $scope.filter.quote_id || null,
					category_id : $scope.filter.category_id || null,
					user_id : $scope.filter.user_id || null
				};

				$http({'url' : '/api/common/files/getAllInfoForUploader', 'method' : 'get', 'params' : params})
					.success(function(data){
						...
						$scope.items = _.union($scope.items, data.items.docs);
						$scope.pagination.hasMorePages = data.items.hasMorePages;
						$scope.loading = false;
					});
			};
						...
						$scope.items.push(
							{
								...
								fileDate : response.file.created_at,
								fileToken : response.file.token,
			...
			$scope.removeFile = function(item){
				$scope.items = angular.copy(_.without($scope.items, item));

				HttpService.delete('/api/quoteFiles/' + item.id);
			};

			$scope.setQuote = function(item) {
				HttpService.post('/api/quoteFiles/setQuote', item)
			...
			$scope.setCategory = function(item) {
				HttpService.post('/api/quoteFiles/setCategory', item)
					.success(function(resp) {
						//
					});
			};
	...
	function avatar() {
		...
		function controllerFunc($scope, $http, FileUploader, FilesService, ngDialog) {
			...
			$scope.uploader = new FileUploader({
				url: '/api/common/files?type='+$scope.fileType+($scope.filePid ? '&pid=' + $scope.filePid : '')+($scope.previewsize ? '&preview_size=' + $scope.previewsize : '')+($scope.previewsize ? '&preview_ext=' + $scope.previewext : ''),
				headers : {'X-CSRF-TOKEN' : BACKEND_CFG.CSRF_TOKEN},
				alias: 'qqfile',
				autoUpload: true,
				removeAfterUpload: true,
