	...
	function FileService(BACKEND_CFG) {
		...
		this.src = function (type, token, size, ext) {
			...
			if(typeof token !== 'undefined' && token) {
				...
				return ['', cfg.assets_dir, cfg.dir[type], token, size + '.' + (ext ? ext : (cfg.sizes[type][size].format || 'jpg') )].join('/');
			} else {
...
//		};
	...
	function SeoService($rootScope) {
		var title = 'Streaming,Media',
			description = 'CCC! Streaming Media';
